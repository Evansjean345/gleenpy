...
export default function ItemsNetflix() {
  ...
  useEffect(() => {
    const fetchSeries = async () => {
      try {
        ...
        const netflixSeries = response.data.filter(
          (serie) => serie.type === "netflix"
        );
        const shuffledSeries = shuffleArray(netflixSeries); // Mélanger les films
